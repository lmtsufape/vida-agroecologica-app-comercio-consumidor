...
import 'package:vidaagroconsumidor/shared/core/repositories/produto_repository.dart';
...
class ProductsController extends GetxController {
  ...
  Future<void> loadProdutos([int? bancaId]) async {
    try {
      isLoading(true);
      produtos1 = await produtoRepository.getProdutos(bancaId ?? 0);
      _produtos = produtos1;
  ...
  List<int> getAvailableCategories() {
    ...
    for (int i = 0; i < categories.length; i++) {
      if (_produtos.any((produto) =>
        ...
        availableCategoryIndexes.add(i + 1);
      }
    }
